...
public class BlockStateHandlerImpl extends BlockStateHandler<BlockState> {
	...
	public List<String> listTags(BlockState stack) {
		return BlockTags.getCollection().getOwningTags(stack.getBlock()).stream().map(e -> "#" + e).collect(Collectors.toList());
	}
	...
	public List<com.tom.cpl.block.BlockState> listNativeEntries(String tag) {
		...
		if (tag.charAt(0) == '#') {
			...
			if (rl != null) {
				ITag<Block> i = BlockTags.getCollection().get(rl);
				i.getAllElements().stream().map(b -> wrap(b.getDefaultState())).forEach(stacks::add);
			}
		} else {
			ResourceLocation rl = ResourceLocation.tryCreate(tag);
			Block item = Registry.BLOCK.getOrDefault(rl);
			if (item != null) {
	...
	public List<String> listNativeTags() {
		return BlockTags.getCollection().getRegisteredTags().stream().map(ResourceLocation::toString).collect(Collectors.toList());
	}

	@Override
	public List<com.tom.cpl.block.BlockState> getAllElements() {
		return Registry.BLOCK.stream().map(b -> wrap(b.getDefaultState())).collect(Collectors.toList());
	}
