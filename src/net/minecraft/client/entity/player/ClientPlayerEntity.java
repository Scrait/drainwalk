...
public class ClientPlayerEntity extends AbstractClientPlayerEntity implements CapeHolder
{
    ...
    public void tick()
    {
        ...
        {
            final UpdateEvent eventUpdate = new UpdateEvent();
    ...
    private void onUpdateWalkingPlayer()
    {
        ...
        final EventEntitySync ees = new EventEntitySync(this.rotationYaw, this.rotationPitch, this.getPosX(), this.getPosY(), this.getPosZ(), this.onGround, flag);
        EventManager.call(ees);

        simulate(this);

        if (!ees.isCancelled()) {
            if (ees.isSprint() != this.serverSprintState) {
                CEntityActionPacket.Action centityactionpacket$action = ees.isSprint() ? CEntityActionPacket.Action.START_SPRINTING : CEntityActionPacket.Action.STOP_SPRINTING;
                this.connection.sendPacket(new CEntityActionPacket(this, centityactionpacket$action));
                this.serverSprintState = ees.isSprint();
            }
            ...
            if (this.isCurrentViewEntity()) {
                double d4 = ees.getPosX() - this.lastReportedPosX;
                double d0 = ees.getPosY() - this.lastReportedPosY;
                double d1 = ees.getPosZ() - this.lastReportedPosZ;
                double d2 = (double) (ees.getYaw() - this.lastReportedYaw);
                double d3 = (double) (ees.getPitch() - this.lastReportedPitch);
                ...
                if (this.isPassenger()) {
                    ...
                    this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(vector3d.x, -999.0D, vector3d.z, ees.getYaw(), ees.getPitch(), ees.isOnGround()));
                    flag1 = false;
                } else if (flag1 && flag2) {
                    this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(ees.getPosX(), ees.getPosY(), ees.getPosZ(), ees.getYaw(), ees.getPitch(), ees.isOnGround()));
                } else if (flag1) {
                    this.connection.sendPacket(new CPlayerPacket.PositionPacket(ees.getPosX(), ees.getPosY(), ees.getPosZ(), ees.isOnGround()));
                } else if (flag2) {
                    this.connection.sendPacket(new CPlayerPacket.RotationPacket(ees.getYaw(), ees.getPitch(), ees.isOnGround()));
                } else if (this.prevOnGround != ees.isOnGround()) {
                    this.connection.sendPacket(new CPlayerPacket(ees.isOnGround()));
                }

                if (flag1) {
                    this.lastReportedPosX = ees.getPosX();
                    this.lastReportedPosY = ees.getPosY();
                    this.lastReportedPosZ = ees.getPosZ();
                    this.positionUpdateTicks = 0;
                }

                if (flag2) {
                    this.lastReportedYaw = ees.getYaw();
                    this.lastReportedPitch = ees.getPitch();
                }

                this.prevOnGround = ees.isOnGround();
    ...
    {
        final NoPushEvent npe = new NoPushEvent(NoPushEvent.PushType.Blocks);
        EventManager.call(npe);
    ...
    public void livingTick()
    {
        ...
        {
            final EventNoSlow ens = new EventNoSlow();
            EventManager.call(ens);
            if (!ens.isCancelled()) {
    ...
    public void move(MoverType typeIn, Vector3d pos)
    {
        ...
        final EventMove em = new EventMove(pos.x, pos.y, pos.z);
        EventManager.call(em);
        super.move(typeIn, new Vector3d(em.getX(), em.getY(), em.getZ()));
