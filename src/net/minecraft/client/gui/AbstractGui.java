...
public abstract class AbstractGui
{
    ...
    protected void fillGradient(MatrixStack matrixStack, int x1, int y1, int x2, int y2, int colorFrom, int colorTo) {
        fillGradient(matrixStack, x1, y1, x2, y2, this.blitOffset, colorFrom, colorFrom, colorTo, colorTo);
    }
    ...
    protected static void fillGradient(MatrixStack matrixStack, int x1, int y1, int x2, int y2, int blitOffset, int colorFrom, int colorTo, int colorFrom2, int colorTo2)
    {
        ...
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_COLOR);
        fillGradient(matrixStack.getLast().getMatrix(), bufferbuilder, x1, y1, x2, y2, blitOffset, colorFrom, colorTo, colorFrom2, colorTo2);
        tessellator.draw();
    ...
    {
        fillGradient(matrix, builder, x1, y1, x2, y2, z, colorA, colorA, colorB, colorB);
    }
    ...
    {
        final float[] cA = ColorService.getRGBAf(colorA);
        final float[] cB = ColorService.getRGBAf(colorB);
        final float[] cC = ColorService.getRGBAf(colorC);
        final float[] cD = ColorService.getRGBAf(colorD);
        builder.pos(matrix, (float)x2, (float)y1, (float)z).color(cA[0], cA[1], cA[2], cA[3]).endVertex();
        builder.pos(matrix, (float)x1, (float)y1, (float)z).color(cB[0], cB[1], cB[2], cB[3]).endVertex();
        builder.pos(matrix, (float)x1, (float)y2, (float)z).color(cC[0], cC[1], cC[2], cC[3]).endVertex();
        builder.pos(matrix, (float)x2, (float)y2, (float)z).color(cD[0], cD[1], cD[2], cD[3]).endVertex();
    }
