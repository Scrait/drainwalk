...
import tech.drainwalk.client.modules.render.ViewModel;
...
public class FirstPersonRenderer
{
    ...
    {
        int i = handIn == HandSide.RIGHT ? 1 : -1;
        final ViewModel viewModel = Drainwalk.getInstance().getApiMain().getModuleManager().findByClass(ViewModel.class);
        if (viewModel.isEnabledThis() && viewModel.isEnabled()) {
            if (i == 1) {
                matrixStackIn.translate(viewModel.right_x.getValue(),
                        viewModel.right_y.getValue(), viewModel.right_z.getValue());
                if (viewModel.anim.getValue()) {
                    matrixStackIn.rotate(Vector3f.XP.rotationDegrees(-105));
    ...
    private void renderItemInFirstPerson(AbstractClientPlayerEntity player, float partialTicks, float pitch, Hand handIn, float swingProgress, ItemStack stack, float equippedProgress, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn))
        {
            ...
            else
            {
                ...
                {
                    final ViewModel viewModel = Drainwalk.getInstance().getApiMain().getModuleManager().findByClass(ViewModel.class);
                    if (!(viewModel.isEnabledThis() && viewModel.isEnabled() && viewModel.anim.getValue())) {
