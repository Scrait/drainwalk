...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable
{
    ...
    private float getFOVModifier2(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFOVSetting)
    {
        if (this.debugView) {
            return 90.0F;
        } else {
            ...
            if (useFOVSetting) {
                f = (float) this.mc.gameSettings.fov;
            ...
            if (this.mc.currentScreen == null) {
                flag = mc.gameSettings.ofKeyBindZoom.isKeyDown();
            }
            ...
            if (entity instanceof LivingEntity && ((LivingEntity) entity).getHealth() <= 0.0F) {
                float f1 = (float) ((LivingEntity) entity).deathTime + partialTicks;
    ...
    {
        final EventGameOverlay ego = new EventGameOverlay(EventGameOverlay.OverlayType.Hurt);
        EventManager.call(ego);
    ...
    public void renderWorld(float partialTicks, long finishTimeNano, MatrixStack matrixStackIn)
    {
        ...
        if (this.renderHand && !Shaders.isShadowPass)
        {
            ...
            final EventRender3D er3d = new EventRender3D.PreHand(matrixStackIn, partialTicks);
            EventManager.call(er3d);
        ...
        final EventRender3D er3dDefault = new EventRender3D(matrixStackIn, partialTicks);
        EventManager.call(er3dDefault);
        this.mc.getProfiler().endSection();
    }

    public void sukazaebalrabotai(MatrixStack stack, float partialTicks) {
        this.farPlaneDistance = (float) (this.mc.gameSettings.renderDistanceChunks * 16);
        ...
        this.hurtCameraEffect(stack, partialTicks);
        ...
        if (this.mc.gameSettings.getPointOfView() == PointOfView.THIRD_PERSON_FRONT)
        {
        ...
        GlStateManager.rotatef(mc.player.prevRotationYaw + (mc.player.rotationYaw - mc.player.prevRotationYaw) * partialTicks + (180.0F), 0.0F, 1.0F, 0.0F);
        //GlStateManager.translatef(0.0F, -mc.player.getEyeHeight(), 0.0F);
//        float f6 = mc.player.prevRotationYaw + (mc.player.rotationYaw - mc.player.prevRotationYaw) * partialTicks
//                + 180.0F;
//
//        float f7 = mc.player.prevRotationPitch + (mc.player.rotationPitch - mc.player.prevRotationPitch) * partialTicks;
//        float f8 = 0.0F;
//
//        if (mc.gameSettings.getPointOfView() == PointOfView.THIRD_PERSON_FRONT) {
//            GlStateManager.rotatef(180.0F, 0, 1.0F, 0.0F);
//        }
//
//        GlStateManager.rotatef(f8, 0.0F, 0.0F, 1.0F);
//        GlStateManager.rotatef(f7, 1.0F, 0.0F, 0.0F);
//        GlStateManager.rotatef(f6, 0.0F, 1.0F, 0.0F);
    }
    ...
    {
        final EventGameOverlay ego = new EventGameOverlay(EventGameOverlay.OverlayType.TotemPop);
        EventManager.call(ego);
