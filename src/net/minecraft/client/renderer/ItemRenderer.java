...
public class ItemRenderer implements IResourceManagerReloadListener
{
    ...
    public void renderItemModelIntoGUI3D(ItemStack stack, int x, int y)
    {
        renderItemGui = true;
        RenderSystem.pushMatrix();
        this.textureManager.bindTexture(AtlasTexture.LOCATION_BLOCKS_TEXTURE);
        ...
        this.renderItem(stack, ItemCameraTransforms.TransformType.GUI, false, matrixstack, irendertypebuffer$impl, 15728880, OverlayTexture.NO_OVERLAY, this.getItemModelWithOverrides(stack, (World)null, (LivingEntity)null));
        irendertypebuffer$impl.finish();
        RenderSystem.enableDepthTest();
    ...
    private void renderItemIntoGUI(@Nullable LivingEntity livingEntity, ItemStack stack, int x, int y)
    {
        if (!stack.isEmpty())
        {
            ...
            catch (Throwable throwable)
            {
                ...
                {
                    return String.valueOf(stack.getItem().getName());
                });
