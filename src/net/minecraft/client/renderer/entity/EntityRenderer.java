...
public abstract class EntityRenderer<T extends Entity> implements net.optifine.entity.model.IEntityRenderer
{
    ...
    {
        final NameTags nameTags = Drainwalk.getInstance().getApiMain().getModuleManager().findByClass(NameTags.class);
        if (nameTags.isEnabled() && nameTags.widgets.isSelected("Name") && Minecraft.getInstance().player.getDistance(entityIn) > 4) return;
