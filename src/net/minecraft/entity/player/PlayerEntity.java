...
public abstract class PlayerEntity extends LivingEntity
{
    ...
    {
        final NoPushEvent npe = new NoPushEvent(NoPushEvent.PushType.Water);
    ...
    public Vector3d getLeashPosition(float partialTicks)
    {
        ...
        if (!this.isElytraFlying() && !this.isSpinAttacking())
        {
            ...
            {
                return this.getLerpedPos(partialTicks).add((new Vector3d(d0, 0.2D, -0.15D)).rotatePitch(-f).rotateYaw(-f1));
            }
            else
            {
                double d5 = this.getBoundingBox().getYSize() - 1.0D;
                double d6 = this.isCrouching() ? -0.2D : 0.07D;
                return this.getLerpedPos(partialTicks).add((new Vector3d(d0, d5, d6)).rotateYaw(-f1));
            }
        }
        else
        {
            ...
            return this.getLerpedPos(partialTicks).add((new Vector3d(d0, -0.11D, 0.85D)).rotateRoll(-f2).rotatePitch(-f).rotateYaw(-f1));
        }
    }
